 ...
 *      这是由于镜面无法在地图边缘同时出现两个。
 *   (3.你可以直接画一张大的镜面图，用于特殊的解谜，将密码写在镜面上。
 * 同步镜像：
 *   (1.同步镜像能支持大部分动作效果，包括跳跃、透明度、多帧行走图。
 *   (2.所有事件都默认开启镜像反射，并且与事件的透明度一致，通过事件复制器
 *      生成的新事件也支持镜像反射。
 *   (3.同步镜像不支持滤镜、粉碎效果。
 ...
 * 插件指令：>图块同步镜像 : 10 : 不反射镜像
 * 插件指令：>图块同步镜像 : 10 : 开启反射镜像
 *
 * 1.数字表示事件id。
 ...
 * 插件指令：>图块同步镜像 : 10 : 关闭镜像透明同步
 * 插件指令：>图块同步镜像 : 10 : 开启镜像透明同步
 *
 * 1.数字表示事件id。
...
Spriteset_Map.prototype.drill_LSR_createReflect = function() {
	...
	this._drill_LSR_layer.z = 1;									//z轴：1.事件下方 3.事件相同 5.事件上方 6.影子 7.气泡 8.动画层 9.鼠标目的地
...
Drill_Sprite_LSR.prototype.update = function() {
	...
	if( this._character._transparent == true ){ this.visible = false; }	//似乎很多插件都影响了这个透明度设置
