 ...
 * @on 跳过
 * @off 关闭
 * @desc 只有一个角色时，点击 技能、装备、状态 时，不用选角色头像，直接进入。流程可以去文档"关于全自定义主菜单界面.docx"看看。
 * @default false
 * 
 * @param 角色头像按钮组
 * @parent ----角色头像按钮----
...
if( Imported.Drill_CoreOfWindowAuxiliary &&
	...
	for( var i = 0; i < DrillUp.g_SMa_styleList_length; i++ ){
		if( DrillUp.parameters["角色固定框样式-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_SMa_styleList[i] = JSON.parse(DrillUp.parameters["角色固定框样式-" + String(i+1) ]);
			DrillUp.g_SMa_styleList[i] = DrillUp.drill_SMa_initActorBoardStyle( DrillUp.g_SMa_styleList[i] );
	...
	for( var i = 0; i < DrillUp.g_SMa_bind_length; i++ ){
		if( DrillUp.parameters["角色框设置-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_SMa_bind[i] = JSON.parse(DrillUp.parameters["角色框设置-" + String(i+1) ]);
			DrillUp.g_SMa_bind[i] = DrillUp.drill_SMa_initActorBoardBind( DrillUp.g_SMa_bind[i] );
...
Scene_Menu.prototype.drill_SMa_createCommandButton = function() {
	...
	// > 准备按钮组参数
	var data_style = JSON.parse(JSON.stringify( DrillUp.g_COSB_btn[$gameSystem._drill_SMa_commandButton_index] ));	//深拷贝数据
