 ...
 *      但是要注意不能死循环嵌套。
 *   (3.了解更多窗口字符，可以去看看"关于窗口字符.docx"。
 * 设计：
 *   (1.你可以通过该插件来简化一个冗长的通用故事的字符串。
 *      也可以用来表示一个名字、一个通知、一个专有名词。
 * 
 * -----------------------------------------------------------------------------
 * ----激活方式
 * 
 * 指代字符
 *   \str[1]     数字1表示对应配置的第1个字符串。
 *   (str全称为：String，即字符串)
 * 
 * 你可以在窗口、对话框中设置指代字符，配置后将显示相应的字符串。
 * 
 * -----------------------------------------------------------------------------
 * ----脚本相关
 * 默认rmmv中常用脚本如下：
...
//
//		★必要注意事项：
//			1.这里相对完整地复刻了rmmv变量、开关的程序结构，将字符串对象化。
//			
//			
//		★其它说明细节：
//			1.
...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.drill_COSt_initString = function( dataFrom ) {
		var data = {};
		var temp = String( dataFrom["字符串内容"] || "" );
