...
Game_System.prototype.drill_SSpH_dataInit = function() {
	this._drill_SSpH_context_list = JSON.parse(JSON.stringify( DrillUp.g_SSpH_context_list ));	//拷贝object（杜绝引用造成的修改）
...
// * 信息面板H - 描述图片刷新
//==============================
Scene_Drill_SSpH.prototype.drill_refreshDescPic = function(index) {
	
	// > 资源预加载
	if( this._window_desc_pic._drill_bitmaps.length == 0 ){
		var visible_list = $gameTemp._drill_SSpH_visibleList;
		for( var i=0; i < visible_list.length; i++ ){
			var context_index = visible_list[i]['index'];
			this._window_desc_pic._drill_bitmaps[i] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpH_context_list[context_index]["pic"]);	
		}
	}
...
// * 显示窗口 - 重绘内容
//==============================
Drill_SSpH_DescWindow.prototype.drill_refreshDesc = function(index) {
	// > 获取当前选项的描述内容
	var cur_index = index;
	var temp_data = $gameTemp._drill_SSpH_visibleList[ cur_index ];
	var context_list = temp_data['context'];
	if( temp_data['locked'] ){
		context_list = DrillUp.g_SSpH_locked_context;
	}
