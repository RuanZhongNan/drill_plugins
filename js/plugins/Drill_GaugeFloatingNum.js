 ...
 * 插件指令：>地图漂浮数字 : 位置[10,10] : 样式[1] : +10 : 持续时间[20]
 * 插件指令：>地图漂浮数字 : 位置变量[25,26] : 样式[1] : +10 : 持续时间[20]
...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 0; i < DrillUp.g_GFN_button_length; i++) {
		if( DrillUp.parameters["漂浮数字样式-" + String(i+1) ] != undefined &&
			...
			DrillUp.g_GFN_button[i] = JSON.parse(DrillUp.parameters["漂浮数字样式-" + String(i+1) ]);
			DrillUp.g_GFN_button[i] = DrillUp.drill_GFN_initParam( DrillUp.g_GFN_button[i] );
		}else{
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if(command === ">地图漂浮数字"){
		...
		if( args.length == 8 ){				//>地图漂浮数字 : 玩家 : 样式[1] : +10 : 持续时间[20]
			...
			var pos = null;
			var e_pos = null;
			if( pos_str.indexOf("位置变量[") != -1 ){
				pos_str = pos_str.replace("位置变量[","");
			...
			if( pos_str.indexOf("位置[") != -1 ){
				pos_str = pos_str.replace("位置[","");
				pos_str = pos_str.replace("]","");
				var temp_arr = pos_str.split(/[,，]/);
				if( temp_arr.length >= 2 ){
...
// ** 数字贴图
// 
// 			说明：这里将漂浮数字的各个状态封装在一起。
//				  初始化设置资源后，调用接口状态切换，能够随时切换到指定的漂浮数字状态。
//=============================================================================
//==============================
// * 数字贴图 - 定义
//==============================
...
Drill_GFN_NumberSprite.prototype.drill_initData = function() {
	...
	var style_id = this._drill_data['style_id'];
...
Drill_GFN_NumberSprite.prototype.drill_initSprite = function() {
	...
	if( data['opacity_mode'] == "瞬间消失" ){				//（透明度这里直接固定配置内容）
		ballistics_opacity['opacityTime'] = data['sustain_time'];					//时间同步
		ballistics_opacity['opacityType'] = "固定数值";								//固定数值
		ballistics_opacity['opacityFix'] = 255;										//
	}
	if( data['opacity_mode'] == "线性消失" ){		
		ballistics_opacity['opacityTime'] = data['sustain_time'];					//时间同步
		ballistics_opacity['opacityType'] = "线性变化";								//固定线性变化
		ballistics_opacity['opacityTarget'] = 0 ;									//目标透明度（注意，与反向无关，反向=倒放）
		ballistics_opacity['opacityDelay'] = 0 ;									//变化延迟
		ballistics_opacity['opacityTranTime'] = data['sustain_time'] ;				//变化时长
	}
	if( data['opacity_mode'] == "等一半时间后线性消失" ){	
		ballistics_opacity['opacityTime'] = data['sustain_time'];					//时间同步
		ballistics_opacity['opacityType'] = "线性变化";								//固定线性变化
		ballistics_opacity['opacityTarget'] = 0 ;									//目标透明度（注意，与反向无关，反向=倒放）
		ballistics_opacity['opacityDelay'] = data['sustain_time']/2 ;				//变化延迟
		ballistics_opacity['opacityTranTime'] = data['sustain_time']/2 ;			//变化时长
	}
	$gameTemp.drill_COBa_setBallisticsOpacity( ballistics_opacity );			//弹道核心 - 透明度初始化
	$gameTemp.drill_COBa_preBallisticsOpacity( this, 0, 255 );					//弹道核心 - 透明度推演
