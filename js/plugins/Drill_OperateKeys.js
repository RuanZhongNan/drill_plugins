 ...
 *   直接修改控制台的所有手柄、键盘按键。
 * 2.基本键的键位必须不重复，重复会出现按键失效情况。
 * 3.你可以设置扩展键的几个键为同一个键位，节约按键，但是要注意对应键
 *   起效的时机，场景等情况。
 * 4.手柄因为键位少，所以固定了功能键的设置。
 *   键盘就可以完全灵活设置，任意一个或两个键都可以触发，并且几个键可以
 *   作用于同一个功能。
 * 5.你可以通过插件指令设置方向键翻转，做成arpg的"混乱"状态效果。
 * 6.注意，该插件有可能覆盖掉其他插件的某些自定义按键设置，你需要调整
 *   一下插件的位置。如果按键仍然不起效果，你只能做插件取舍了。
 *
 * -----------------------------------------------------------------------------
 * ----特殊键盘按键
 * 你可以设置特殊的键盘按键，可以填入以下字符关键字：
