...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === ">信息面板L") {
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpL_context_list_length; i++){
					$gameSystem.drill_SSpL_initDataIfNeed();
					$gameSystem._drill_SSpL_context_list[i]['enabled'] = true;	//正常存储
				}
			}
			if( type == "隐藏全部" ){
				for( var i = 1; i <= DrillUp.g_SSpL_context_list_length; i++){
					$gameSystem.drill_SSpL_initDataIfNeed();
					$gameSystem._drill_SSpL_context_list[i]['enabled'] = false;	//正常存储
				}
			}
			if( type == "锁定全部" ){
				for( var i = 1; i <= DrillUp.g_SSpL_context_list_length; i++){
					$gameSystem.drill_SSpL_initDataIfNeed();
					$gameSystem._drill_SSpL_context_list[i]['locked'] = true;	//正常存储
				}
			}
			if( type == "解锁全部" ){
				for( var i = 1; i <= DrillUp.g_SSpL_context_list_length; i++){
					$gameSystem.drill_SSpL_initDataIfNeed();
					$gameSystem._drill_SSpL_context_list[i]['locked'] = false;	//正常存储
				}
			}
		}
	}
	if (command === ">信息面板L") {
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				$gameSystem.drill_SSpL_initDataIfNeed();
				$gameSystem._drill_SSpL_context_list[temp1]['enabled'] = true;	//正常存储
			}
			if( type == "隐藏选项" ){
				$gameSystem.drill_SSpL_initDataIfNeed();
				$gameSystem._drill_SSpL_context_list[temp1]['enabled'] = false;	//正常存储
			}
			if( type == "锁定选项" ){
				$gameSystem.drill_SSpL_initDataIfNeed();
				$gameSystem._drill_SSpL_context_list[temp1]['locked'] = true;	//正常存储
			}
			if( type == "解锁选项" ){
				$gameSystem.drill_SSpL_initDataIfNeed();
				$gameSystem._drill_SSpL_context_list[temp1]['locked'] = false;	//正常存储
			}
			if( type == "选中页" ){
				var temp = temp1 -1;
				if( temp < 0 ){ temp = 0; };
				if( temp > $gameTemp._drill_SSpL_visibleList.length -1 ){ temp = $gameTemp._drill_SSpL_visibleList.length -1; };
				$gameSystem._drill_SSpL_context_index = temp;
			}
		}
	}
...
Scene_Drill_SSpL.prototype.initialize = function() {
	...
	this._cur_index = -1;
	//alert($gameSystem._drill_SSpL_context_list); //检查存档是否有记录
	$gameSystem.drill_SSpL_initDataIfNeed();
};
//==============================
// * 信息面板L - 读取 正常存储/全局存储 的值
//==============================
Game_System.prototype.drill_SSpL_initDataIfNeed = function() {
	if( $gameSystem._drill_SSpL_context_list != undefined ){ return; }
	
	this._drill_SSpL_context_list = JSON.parse(JSON.stringify( DrillUp.g_SSpL_context_list ));	//深拷贝数据（杜绝引用造成的修改）
};
//==============================
// * 信息面板L - 创建
//==============================
...
// * 信息面板L - 重设窗口起点（切换选项时）
//==============================
Scene_Drill_SSpL.prototype.resetPosition = function() {
...
Scene_Drill_SSpL.prototype.drill_refreshDescPic = function( cur_index ) {
	...
	if( src_tank.length == 0 ){
		...
		for( var i=0; i < temp_list.length; i++ ){
			...
			src_tank[ i+1 ] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpL_context_list[context_index]["pic"]);	
		}
...
// * 帧刷新 - 描述图片
//==============================
Scene_Drill_SSpL.prototype.updateDescPic = function() {
	if( DrillUp.g_SSpL_descPic_showInstant == false ){
		this._window_desc_pic.opacity += 255/DrillUp.g_SSpL_descPic_slideAnim['slideTime'];
	}
}
//==============================
// * 帧刷新 - 窗口选项刷新
//==============================
Scene_Drill_SSpL.prototype.updateIndex = function() {
	if( $gameSystem._drill_SSpL_context_index != undefined || $gameSystem._drill_SSpL_context_index != null ){
		this._window_select.select( $gameSystem._drill_SSpL_context_index );
		$gameSystem._drill_SSpL_context_index = null;		//设置选中页
	}
...
Drill_SSpL_SelectWindow.lastTopRow = 0;		//上一次的顶层范围项
Drill_SSpL_SelectWindow.lastIndex  = 0;		//上一次的选中项
//==============================
// * 选项窗口 - 初始化
//==============================
...
Drill_SSpL_SelectWindow.prototype.refresh = function() {
	...
	for(var i=0; i < DrillUp.g_SSpL_context_list_length; i++){
		...
		var temp = $gameSystem._drill_SSpL_context_list[i+1];	//正常存储
		
		if( temp != "" && temp['enabled'] == true ){
			$gameTemp._drill_SSpL_visibleList.push( temp );
	...
	for( var j=0; j< $gameTemp._drill_SSpL_visibleList.length ;j++ ){
		if( $gameTemp._drill_SSpL_visibleList[j]['locked'] == true ){
			this._list.push( DrillUp.g_SSpL_locked_name );
			continue;
		}
		
		if( DrillUp.g_SSpL_command_window['nameExEnabled'] == true ){
			this._list.push( $gameTemp._drill_SSpL_visibleList[j]['nameEx'] );
			continue;
		}
		
		this._list.push( $gameTemp._drill_SSpL_visibleList[j]['name'] );
	}
...
if( Imported.MOG_MenuCursor == true ){
	...
	Drill_SSpL_SelectWindow.prototype.need_set_mcursor_data = function() {
		if( DrillUp.g_SSpL_command_window['menuCursorEnabled'] == false ){
			return false;
		}
...
if( Imported.MOG_CursorBorder == true ){
	...
	Drill_SSpL_SelectWindow.prototype.createSprSelMenu = function() {
		if( DrillUp.g_SSpL_command_window['menuCursorBorderEnabled'] == false ){
			return ;
		}
