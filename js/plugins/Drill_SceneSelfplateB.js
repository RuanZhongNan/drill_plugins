...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.g_SSpB_selWin_menuCursorEnabled = String( DrillUp.parameters['是否启用mog菜单指针'] || "true") == "true";
	DrillUp.g_SSpB_selWin_menuCursorBorderEnabled = String( DrillUp.parameters['是否启用mog菜单边框'] || "true") == "true";
		...
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpB_context_list_length; i++){
					...
					$gameSystem.drill_SSpB_initDataIfNeed();
...
//==============================
// * 信息面板B - 读取 正常存储/全局存储 的值
//==============================
Game_System.prototype.drill_SSpB_initDataIfNeed = function() {
	if( $gameSystem._drill_SSpB_context_list != undefined ){ return; }
...
Drill_SSpB_SelectWindow.prototype.refresh = function() {
	...
	for( var j=0; j< $gameTemp._drill_SSpB_visibleList.length ;j++ ){
		if( $gameTemp._drill_SSpB_visibleList[j]['locked'] == true ){
			this._list.push( DrillUp.g_SSpB_locked_name );
			continue;
		}
		
		if( DrillUp.g_SSpB_selWin_nameExEnabled == true ){
			this._list.push( $gameTemp._drill_SSpB_visibleList[j]['nameEx'] );
			continue;
		}
...
if( Imported.MOG_MenuCursor == true ){
	...
	Drill_SSpB_SelectWindow.prototype.need_set_mcursor_data = function() {
		if( DrillUp.g_SSpB_selWin_menuCursorEnabled == false ){
			return false;
		}
...
if( Imported.MOG_CursorBorder == true ){
	...
	Drill_SSpB_SelectWindow.prototype.createSprSelMenu = function() {
		if( DrillUp.g_SSpB_selWin_menuCursorBorderEnabled == false ){
			return ;
		}
