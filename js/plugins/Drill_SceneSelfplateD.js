...
Scene_Drill_SSpD.prototype.drill_refreshDescPic = function(index) {
	...
	if( this._window_desc_pic._drill_bitmaps.length == 0 ){
		var visible_list = $gameTemp._drill_SSpD_visibleList;
		for( var i=0; i < visible_list.length; i++ ){
			var context_index = visible_list[i]['index'];
			this._window_desc_pic._drill_bitmaps[i] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpD_context_list[context_index]["pic"]);	
		}
	}
