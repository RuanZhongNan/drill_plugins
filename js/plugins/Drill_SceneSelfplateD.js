...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === '>信息面板D') {
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.g_SSpD_context_list[i]['enabled'] = true;	//全局数据
					if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
					$gameSystem._drill_SSpD_context_list[i]['enabled'] = true;	//存档数据
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "隐藏全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.g_SSpD_context_list[i]['enabled'] = false;	//全局数据
					if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
					$gameSystem._drill_SSpD_context_list[i]['enabled'] = false;	//存档数据
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "锁定全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.g_SSpD_context_list[i]['locked'] = true;	//全局数据
					if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
					$gameSystem._drill_SSpD_context_list[i]['locked'] = true;	//存档数据
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "解锁全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.g_SSpD_context_list[i]['locked'] = false;	//全局数据
					if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
					$gameSystem._drill_SSpD_context_list[i]['locked'] = false;	//存档数据
				}
				DataManager.forceSaveGlobalInfo();
			}
		}
	...
	if (command === '>信息面板D') {
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				DrillUp.g_SSpD_context_list[temp1]['enabled'] = true;	//全局数据
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
				$gameSystem._drill_SSpD_context_list[temp1]['enabled'] = true;	//存档数据
			}
			if( type == "隐藏选项" ){
				DrillUp.g_SSpD_context_list[temp1]['enabled'] = false;	//全局数据
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
				$gameSystem._drill_SSpD_context_list[temp1]['enabled'] = false;	//存档数据
			}
			if( type == "锁定选项" ){
				DrillUp.g_SSpD_context_list[temp1]['locked'] = true;	//全局数据
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
				$gameSystem._drill_SSpD_context_list[temp1]['locked'] = true;	//存档数据
			}
			if( type == "解锁选项" ){
				DrillUp.g_SSpD_context_list[temp1]['locked'] = false;	//全局数据
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
				$gameSystem._drill_SSpD_context_list[temp1]['locked'] = false;	//存档数据
			}
			if( type == "选中页" ){
				var temp = temp1 -1;
				if( temp < 0 ){ temp = 0; };
...
// * 信息面板D - 描述图片刷新
//==============================
Scene_Drill_SSpD.prototype.drill_refreshDescPic = function(index) {
	var context_index = $gameTemp._drill_SSpD_visibleList[index]['index'];
	this._window_desc_pic.bitmap = ImageManager.load_MenuSelfDef(DrillUp.g_SSpD_context_list[context_index]["pic"]);
}
...
// * 帧刷新 - 箭头 - 鼠标点击图片范围判断
//==============================
Scene_Drill_SSpD.prototype.drill_isOnSprite = function(sprite) {
...
Drill_SSpD_SelectWindow.prototype.refresh = function() {
	...
	for(var i=1; i<= DrillUp.g_SSpD_context_list_length ;i++){
		...
		if( DrillUp.g_SSpD_title_data_global ){
			var temp = DrillUp.g_SSpD_context_list[i];	//全局数据
		}else{
			var temp = $gameSystem._drill_SSpD_context_list[i];	//存档数据
		}
...
// * 显示窗口 - 重绘内容
//==============================
Drill_SSpD_DescWindow.prototype.drill_refreshDesc = function(index) {
	// > 获取当前选项的描述内容
	var cur_index = index;
	var temp_data = $gameTemp._drill_SSpD_visibleList[ cur_index ];
	var context_list = temp_data['context'];
	if( temp_data['locked'] ){
		context_list = DrillUp.g_SSpD_locked_context;
	}
