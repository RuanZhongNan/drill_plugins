...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.g_SSpG_selWin_menuCursorEnabled = String( DrillUp.parameters['是否启用mog菜单指针'] || "true") == "true";
	DrillUp.g_SSpG_selWin_menuCursorBorderEnabled = String( DrillUp.parameters['是否启用mog菜单边框'] || "true") == "true";
		...
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpG_context_list_length; i++){
					...
					$gameSystem.drill_SSpG_initDataIfNeed();
...
//==============================
// * 信息面板G - 读取 正常存储/全局存储 的值
//==============================
Game_System.prototype.drill_SSpG_initDataIfNeed = function() {
	if( $gameSystem._drill_SSpG_context_list != undefined ){ return; }
...
Drill_SSpG_SelectWindow.prototype.refresh = function() {
	...
	for( var j=0; j< $gameTemp._drill_SSpG_visibleList.length ;j++ ){
		if( $gameTemp._drill_SSpG_visibleList[j]['locked'] == true ){
			this._list.push( DrillUp.g_SSpG_locked_name );
			continue;
		}
		
		if( DrillUp.g_SSpG_selWin_nameExEnabled == true ){
			this._list.push( $gameTemp._drill_SSpG_visibleList[j]['nameEx'] );
			continue;
		}
...
if( Imported.MOG_MenuCursor == true ){
	...
	Drill_SSpG_SelectWindow.prototype.need_set_mcursor_data = function() {
		if( DrillUp.g_SSpG_selWin_menuCursorEnabled == false ){
			return false;
		}
...
if( Imported.MOG_CursorBorder == true ){
	...
	Drill_SSpG_SelectWindow.prototype.createSprSelMenu = function() {
		if( DrillUp.g_SSpG_selWin_menuCursorBorderEnabled == false ){
			return ;
		}
