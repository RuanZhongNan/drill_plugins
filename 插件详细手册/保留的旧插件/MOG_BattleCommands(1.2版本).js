...
Window_ActorCommand.prototype.load_com_images = function() {
	for (var i = 0; i < this._com_list.length; i++) {
		 ...
		 //该区域需要一步一步挖取值
		 //this._com_list[i][0] 表示 (symbol,attack)-节奏攻击
		 //this._com_list[i][1] 表示 (symbol,skill) (ext,1)-物理技
		 //for(var a in this._com_list[i][4]){
		 //textb ="key:"+a+" value:"+this._com_list[i][4][a]+"\n";
		 //alert(textb);
		 //}
		 for (var r = 0; r < this._com_list[i].length; r++) {
			 if( this._com_list[i][r]['symbol'] == 'attack' ){
				this._com_images[i].push(ImageManager.loadBcom(Moghunter.src_com_atk));		//攻击
			 }
			 if( this._com_list[i][r]['symbol'] == 'skill' ){
				this._com_images[i].push(ImageManager.loadBcom(Moghunter.com_list[this._com_list[i][r]['ext']]));		//自定义技能类型
			 }
			 if( this._com_list[i][r]['symbol'] == 'guard' ){
				this._com_images[i].push(ImageManager.loadBcom(Moghunter.src_com_def));		//防御
			 }
			 if( this._com_list[i][r]['symbol'] == 'item' ){
				this._com_images[i].push(ImageManager.loadBcom(Moghunter.src_com_item));	//道具
			 }
			 //注意，顺序是固定的
		 };
	};
