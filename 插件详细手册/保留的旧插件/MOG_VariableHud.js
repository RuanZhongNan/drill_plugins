 ...
 * 插件指令（改最大值）： set_variable_max_n : A : C
 *
 * 参数A：变量框的id号。
 * 参数B：最大值。
 * 参数B：指定变量id的值，作为最大值。
 ...
 * set_variable_max_n : 1 : 2 （设置第一个变量框最大值为 变量2 的值）
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//插件记录：
//		mog新版还是没处理那个插件的bug。新版只是添加了smartfade参数。
//		（整体没变化多少，添加了FileName_Gauge参数。）
//		723行修改了资源图片。（其他地方有几个分号、重复定义var的小细节）
//
//		811行的 $gameSystem._variableHudData[this._index]._visible （那个难以捉摸的bug，变量名不能用visible，换个名字就正常了。）
//
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "hide_variable_hud") {
		...
		if ($gameSystem._variableHudData[varID]) {
	         $gameSystem._variableHudData[varID]._visible = false;
		};
    } else if (command === "show_variable_hud") {
		var varID = Number(args[1]) - 1;
		if ($gameSystem._variableHudData[varID]) {
	         $gameSystem._variableHudData[varID]._visible = true;
		};		
	};
	if (command === "set_variable_max")  {
		var varID = Number(args[1]) - 1;
		if ($gameSystem._variableHudData[varID]) {
	         $gameSystem._variableHudData[varID].maxValue = Number(args[3]);
		};		
	...
	if (command === "set_variable_max_n")  {
		var varID = Number(args[1]) - 1;
		if ($gameSystem._variableHudData[varID]) {
	         $gameSystem._variableHudData[varID].maxValue = $gameVariables.value( Number(args[3]) );
		};
	}
	return true;
};
...
Scene_Map.prototype.createVariableHuds = function() {
	...
	for (var i = 0; i < Moghunter.variableHud_Max; i++) {
		if (!$gameSystem._variableHudData[i]) {
			 ...
			 $gameSystem._variableHudData[i]._visible = vis;
...
VariableHud.prototype.dataSys = function() {	
	if($gameSystem._variableHudData[this._index] == undefined){
		return {};
	}
...
VariableHud.prototype.needHide = function() {
	...
	if ($gameSystem._variableHudData[this._index] != undefined && $gameSystem._variableHudData[this._index]._visible == false) { return true;};
