...
/*:
 * @plugindesc (v1.3)[v1.2]  战斗UI - 角色图像
 ...
 * 插件可以单独使用，显示前视图和背景图。
 * 被扩展：
 *   - Drill_X_BattlePictureChange 战斗UI-角色立绘切换[扩展]
 *     通过该插件提供的支持，你可以切换前视图和背景图。
 * 
 * -----------------------------------------------------------------------------
 * ----设定注意事项
 * 1.插件的作用域：战斗界面。
...
Actor_CMPicture.prototype.actor_cm_refresh = function() {
	...
	if (this._sprite_actor_cm) {
		if (!this._actor_cm_img[actor_id]) {
			this._actor_cm_img[actor_id] = ImageManager.loadPicture(Moghunter.actor_cm_list[actor_id]);
		};
	...
	if (this._sprite_actor_cm_lay) {
		if (!this._actor_cm2_img[actor_id]) {
			this._actor_cm2_img[actor_id] = ImageManager.loadPicture(Moghunter.actor_cm2_list[actor_id]);
		};		
