 ...
 * 
 * @param 举起音效
 * @desc 举起花盆时，播放的音效。填入audio/se文件夹下的某个音效的文件名。
 * @default Jump1
 *
 * @help  
 * =============================================================================
 ...
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 如果设置了举起姿势，那么玩家角色的图片资源需要如下：
 *
 * 角色文件名_pick.png
 *
 * 比如，角色配置了Actor1.png，那么，你需要添加另外的Actor1_pick.png图片
 * 资源。
 *
 * -----------------------------------------------------------------------------
 * ----可选设定
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_PickupThrow');
	...
	Moghunter.pickDirectionButton = String(Moghunter.parameters['Hold Direction'] || 'true');					//这两个变量调整没有效果
	Moghunter.pickDirectionButtonKey = String(Moghunter.parameters['Hold Direction Button'] || 'pagedown');		//
	Moghunter.pickSoundFile = String(Moghunter.parameters['举起音效'] || 'Jump1');
