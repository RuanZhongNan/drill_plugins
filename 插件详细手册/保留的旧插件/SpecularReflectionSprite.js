...
/*:
 * @plugindesc (v1.00)[v1.2] 地图 - 图块镜像反射
 ...
 * =============================================================================
 * 你可以设置图块具有镜像反射效果。
 * 你还可以直接自己绘制镜像区域，具体去看看"关于镜像与阴影.docx"。
...
　　var DrillUp = DrillUp || {}; 
	...
	if( DrillUp.parameters['区域标记'] != "" ){
		DrillUp.ref_areaIds = (JSON.parse( DrillUp.parameters['区域标记'])).map(function(n){ return Number(n) });;
	}else{
		DrillUp.ref_areaIds = ([]).map(function(n){ return Number(n) }); ;
	}
    ...
    Sprite_Specular.prototype.updateVisibility = function() {
		...
		if( !reflectTerrain ){
			reflectTerrain = DrillUp.ref_areaIds.contains(regionId);
		}
        ...
        var isVisible = DrillUp.ref_terrainIds.contains(terrainTag);
		if( !isVisible ){
			isVisible = DrillUp.ref_areaIds.contains(regionId);
		}
		...
		if( DrillUp._map_reflection_data != "" ){
			...
			for(var xx = 0; xx< $gameMap.width() ;xx++){
				for(var yy = 0; yy< $gameMap.height() ;yy++){
					...
					if( DrillUp.ref_terrainIds.contains(terrainTag) ||  DrillUp.ref_areaIds.contains(regionId) ){
